...
Pour plus de simplicité de mise en oeuvre, nous avons choisi d'utiliser les modèles déjà implémentés dans la librairie GSE. Nous vous présentons la modélisation retenue ainsi que les paramètres initiaux de nos simulations.
...
L'unique input de notre modélisation est la courbe de zéro coupons publiée par le régulateur. L'ensemble des autres paramètres, notamment ceux analysés ans la partie portant sur les calculs de sensibilité, ont été choisis à dire d'expert, ou fixés dans des plages. Enfin, l'ensemble des paramètres non-fixés se déduit facielment de la courbe des zéro-coupons.
...
Il est cohérent que la vitesse de convergence soit croissante avec la volatilié. Pousser le nombre de simulations suffirait à corriger cet effet
...
Conformément à nos attentes, les BE convergent bien vers le prix de l'actif en $0$ comme démontré plus haut.

#IV. Conclusion et références

Conformément à la démontration proposée en partie III, le BE reste stable malgré la perturbation de chaque paramètre de notre modèle. Une étude intéressante à mener sera la sensibilité de la vitesse de convergence aux variations des différents paramètres. Mathématiquement, le BE est une espérance (ordre 1). Ainsi nous le calculons avec une approche "juste", que l'on peut caractériser de "prudente" par la courbe de taux fournie par EIOPA. Maintenant, le cadre posé est restreint, et ça, c'est naze. 
