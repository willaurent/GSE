...
L'unique input de notre modélisation est la courbe de zéro coupons publiée par le régulateur. L'ensemble des autres paramètres, notamment ceux analysés dans la partie portant sur les calculs de sensibilité, ont été choisis à dire d'expert, ou fixés dans des plages. Enfin, l'ensemble des paramètres non-fixés se déduit facilement de la courbe des zéro-coupons.
...
D'après le $mathématiques financières$, un critère d'unicité de la mesure martingale en AOA est de définir le drift:
...
Ici $\rho$ est un paramètre des snesibilités de l'actif aux 2 facteurs de risque que sont les 2 mouvements browniens.

Nous avons, par défaut, gardé une valeur positive $\rho = 0.5$, et la volatilité constante $\sigma_bs = 0.2$ 
...
Nous reviendrons plus tard sur la calibration du modèle de taux court et l'influence sur la cohérence avec le marché.
...
Pour ce qui est du modèle de taux forward HJM, on observe que modifier les deux paramètres qui le composent ne change pas les résultats. Une variation de 0.005 fois le prix de l'actif n'est pas suffisamment significative.
...
Conformément à nos attentes, les BE convergent bien vers le prix de l'actif en $0$ comme démontré plus haut. De plus, la majorité des paramètre n'influe que peu sur la vitesse de convergence. Le résultat théorique de convergence du BE vers $S(0)$ est donc clairement confirmé par les simulations.
