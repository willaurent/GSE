...
Nous avons, par défaut, gardé $\rho = 0.5$, et la volatilité constante $\sigma_bs = 0.2$ 
...
Mathématiquement, le BE est une espérance (ordre 1). Ainsi nous le calculons avec une approche "juste", que l'on peut caractériser de "prudente" par l'utilisation de la courbe de taux fournie par EIOPA. La faible complexité du cadre posé permet de tirer des résultats théoriques intéressants et de les confirmer par la simulation, mais il est évident que dans la pratique ce ne serait pas aussi trivial.
